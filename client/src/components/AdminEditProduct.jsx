...
export default function AdminEditProduct({onClose,ProductData,fetchData}) {
    ...
    const handleDeleteProductImage = async(product,index) =>{
        ...
        AXIOS.post("http://localhost:7800/products/delete-product-image",{image:product})
    ...
    const handleSubmit = async(e) =>{
        ...
        const resData = await AXIOS.post("http://localhost:7800/products/update-product",data,{headers:{token:localStorage.getItem('token')}})
        ...
        <div className='bg-white p-4 pb-10 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden'>
            ...
            <form onSubmit={handleSubmit} encType='multipart/form-data' className='grid p-4 gap-2 overflow-y-scroll h-full'>
                ...
                <div className='flex items-center gap-2 overflow-x-scroll scrollbar-none py-5'>      
                    ...
                    <button className='px-3 mt-3 py-2 bg-pink-700 text-white mb-10 hover:bg-pink-900'>Update Product</button>
            </form>
        </div>
