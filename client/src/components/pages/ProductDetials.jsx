      ...
      <div className="min-h-[calc(100vh-64px)] pt-16">
        <div className="container mx-auto p-4">
          <div className="min-h-[200px] flex flex-col lg:flex-row gap-4">
              ...
              <div className="h-full">
                <div className="flex gap-2 lg:flex-col overflow-scroll scrollbar-none h-full">
                  {data.productImage.map((image, index) => {
                    return (
                      <div
                        className="h-20 w-20 bg-white border border-black rounded p-1"
                        key={index}
                      >
                        <img
            ...
            <div>
              <p>
                <span className="bg-pink-200 px-2 py-1 text-pink-800 rounded-full Capit">
                  {data?.ProductBrand}
                </span>
              </p>
              <h2 className="capitalize text-2xl font-medium py-4 lg:text-4xl max-w-[80%] text-wrap">
                {data?.ProductName}
              </h2>
              <p>
              ...
              <div className="flex items-center gap-4">
                <p className="text-black font-extrabold lg:text-3xl text-2xl productFont">
                  {displayINRCurrency(data?.sellingPrice)}
                </p>
                <p className="text-slate-500 lg:text-xl text-base line-through ">
                  {displayINRCurrency(data?.price)}
                </p>
                <p className="px-4 text-4xl font-medium italic text-green-700">
                  -{100 - Math.round((data?.sellingPrice / data?.price) * 100)}%
                </p>
              </div>
              <div className="flex gap-4 items-center my-8">
                <button className="px-3 py-1 border-2 border-green-300 bg-green-600 hover:bg-green-800 text-white font-bold text-lg rounded-full min-w-[180px] ">
                  Buy
                </button>
                <button
                  className="border-2 border-pink-500 px-3 py-1 text-pink-600 hover:text-white hover:bg-pink-600 bg-white font-bold rounded-full min-w-[180px]"
