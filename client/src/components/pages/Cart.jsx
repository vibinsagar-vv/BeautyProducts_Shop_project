            ...
            <div className="flex flex-col lg:flex-row gap-10 lg:justify-between">
              <div className="w-full max-w-3xl ">
                {data.map((product, index) => {
                  return (
                    <div
                      key={index}
                      className="flex w-full h-32 my-2 border bg-white border-black relative"
                    >
                      <div
                        className="absolute right-1 top-1 text-lg rounded-full p-1 font-bold hover:text-white hover:bg-red-600 cursor-pointer"
                        onClick={() => {
                          handleDeleteCartPrduct(product?._id);
                        }}
                      >
                        <IoMdClose />
                      </div>
                      ...
                      <div className="px-4 w-full  py-2 text-start">
                        ...
                        <div className="flex justify-between">
                          <p className="text-base lg:text-lg capitalize font-medium  text-slate-600">
                            {displayINRCurrency(
                              product?.ProductId?.sellingPrice
                            )}
                          </p>
                          <p className="text-base lg:text-lg capitalize font-bold  text-pink-700">
                            {displayINRCurrency(
                              product?.ProductId?.sellingPrice *
                                product?.Quantity
                            )}
              ...
              <div className="w-full flex justify-center p-4">
                <div className="lg:w-[60%] w-full h-52 bg-white border border-black">
                  ...
                  <div className="text-start p-4 flex lg:justify-between">
                    <p>Total Price :</p>
                    <p className="lg:px-4 px-7 font-bold">
                      {displayINRCurrency(TotalPrice)}
                    </p>
                  </div>
                  <button
                    onClick={handleBuy}
                    className="w-full bg-gray-600 py-3 text-white font-bold"
                  >
                    Buy Products
                  </button>
                </div>
