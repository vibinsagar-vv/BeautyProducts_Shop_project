...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-center mx-4">
  <Link to="/product-category/SkinCare" className="flex flex-col items-center">
    <img
      src={SkinCare}
      className="w-40 h-40 rounded-lg border-4 hover:border-primary-light shadow-md hover:shadow-lg"
      alt="SkinCare"
    />
    <span className="text-xl text-black font-semibold">SkinCare</span>
  </Link>
  <Link to="/product-category/Makeup" className="flex flex-col items-center">
    <img
      src={MakeUp}
      className="w-40 h-40 rounded-lg border-4 hover:border-primary-light shadow-md hover:shadow-lg"
      alt="MakeUp"
  ...
  <Link to="/product-category/Hair&Body" className="flex flex-col items-center">
    <img
      src={HairCare}
      className="w-40 h-40 rounded-lg border-4 hover:border-primary-light shadow-md hover:shadow-lg"
      alt="HairCare"
                ...
                {index % 2 == 0 && (
                  <div>
                    <div className="w-full mt-4 flex flex-col md:flex-row  justify-center">
                      <div
                        className={`w-[97%] md:h-80 h-48 ${
                          evenColorClasses[
                            Math.floor(Math.random() * evenColorClasses.length)
                          ][0]
                        } flex justify-between px-10 md:px-36 items-center rounded-md`}
                      >
                        <div>
                          
                          <p className="md:text-7xl text-xl edu_australia font-bold text-white md:w-[500px] my-6 w-[300px] md:ml-20">
                            {name}
                          </p>
                          <div className="w-full flex justify-center mt-12">
                            <button className={`border-4 px-4 bg-white py-1 font-semibold text-lg w-40 rounded-2xl border-white `}>
                              shop now
                            </button>
                          </div>
                        </div>
                        <div className="w-40 md:w-60 relative object-contain h-40 md:h-60 flex">
                        <div className="absolute w-48 h-48 bg-white z-1 rounded-full blur-[60px] opacity-65"></div>
                          {product
                            .filter((product) =>
                              product.subcategory
                                .toLowerCase()
                    ...
                    <div className="w-full mt-4 flex flex-col md:flex-row justify-center">
                      ...
                      >
                        <div className="">
                          ...
                          <p className="md:text-7xl text-nowrap edu_australia text-xl font-bold text-white md:w-[600px] my-6 w-[300px] md:ml-20">
                            {name}
                          </p>
                          <div className="w-full flex justify-center mt-12">
                            <button className={`border-4 px-4 bg-white py-1 font-semibold text-lg w-40 rounded-2xl border-white `}>
                        ...
                        <div className="w-40 md:w-60 relative object-contain h-40 md:h-60 flex">
                          ...
                          {product
                            ...
                            .map((product) => (
                              <img
                                key={product._id} // Use a unique key for each product
                                className="w-40 z-10 md:w-60 h-40 md:h-60 flex"
                                src={`http://localhost:8200/ProductImages/${product.productImage[0]}`}
                                alt={product.name || "Product Image"} // Provide an accessible alt text
                              />
                            ))}
