...
const CarouselTemplate = () => {
  ...
  const goToPreviousSlide = () => {
    const newIndex = currentIndex === 0 ? allBanner.length - 1 : currentIndex - 1;
    setCurrentIndex(newIndex);
  };

  const goToNextSlide = () => {
    const newIndex = currentIndex === allBanner.length - 1 ? 0 : currentIndex + 1;
    setCurrentIndex(newIndex);
  };
    ...
    <div className="relative w-full" aria-label="carousel">
      <div className="relative h-56 overflow-hidden rounded-lg md:h-[500px]">
        {allBanner.map((image, index) => (
          ...
          >
            <img
              src={`http://localhost:7800/Banners/${image.BannerImage}`}
